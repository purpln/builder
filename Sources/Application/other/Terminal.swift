...
func launch(command: String, arguments: [String] = [], path: String? = nil, environment: [String: String] = [:], debug: Bool = false, redirect: Bool = false) throws {
    let success = process(command: command, arguments: arguments, path: path, environment: environment, debug: debug, redirect: redirect) { string in
...
func output(command: String, arguments: [String] = [], path: String? = nil, environment: [String: String] = [:], debug: Bool = false) async -> String {
    await withCheckedContinuation { continuation in
        _ = process(command: command, arguments: arguments, path: path, environment: environment, debug: debug, redirect: true) { string in
            continuation.resume(returning: string.remove("\n"))
        }
    }
}

func bool(command: String, arguments: [String] = [], path: String? = nil, environment: [String: String] = [:], debug: Bool = false, redirect: Bool = false) -> Bool {
    process(command: command, arguments: arguments, path: path, environment: environment, debug: debug, redirect: redirect) { string in
        guard redirect else { return }
        print(string, terminator: "")
    }
}
